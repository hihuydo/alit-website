...
# Last project update: 2026-04-19 — **T0 komplett (43/45 `[x]`, 2 bewusst out-of-scope) + T1 zu 50%+ + T2 Docker-Block 7/8 + Anti-Patterns 9/9 verified**. 14-PR-Session: Sprint D1 CSP Report-Only baseline live (PR #81 — CSP + `/api/csp-report/` + streaming-body-cap + legacy/modern-report-normalization; D2 enforced-flip wartet auf ≥7d clean stream ab 2026-04-19), Sprint E Docker-hardening (PR #82 — cap_drop + no-new-privileges + resource-limits), Sprint F read-only-FS (PR #88 — read_only:true + tmpfs für /tmp + /app/.next/cache), Trivy CI (PRs #86+#87 — filesystem+config+image scan mit `.trivyignore` VEX-whitelist für picomatch CVE-2026-33671), DOMPurify-Audit abgeschlossen (audit-only, no migration — strukturell-rendered Blocks sind primäre XSS-Mitigation). Davor 2026-04-18: Ops-Follow-ups (Branch-Protection, Secret-Scanning, Backup-Cron, DB-REVOKE, `.env` chmod 600, JWT_SECRET+IP_HASH_SALT split staging↔prod). SSH + fail2ban + unattended-upgrades + UFW-bewusst-weg aus 2026-04-17/-18. **Verbleibend:** Tier-1 CSP D2-flip (date-gated), Sprint C Cookie-Migration Phase 2 (date-gated), Tier-2 Slim/Distroless-Base-Image (nominal offen, alpine ist schon slim), Zod (out-of-scope by design), Connection-Pool-Limits (out-of-scope bei Admin-Traffic).
...
- [x] **[Must Have]** `dangerouslySetInnerHTML` nur mit DOMPurify — **auditiert 2026-04-19 (audit-only, no migration)**: primäre XSS-Mitigation ist architektonisch — HTML wird NICHT als Blob in DB gespeichert, Flow ist `RichTextEditor → htmlToBlocks() → structured-blocks-JSONB → JournalBlockRenderer` (kein `dangerouslySetInnerHTML` auf public pages, nur strukturelle Block-Rendering). Hand-rolled `sanitizeHtml()` in RichTextEditor.tsx hat allowlist für tags+attributes, URL-safety via isSafeUrl, iframe-host-whitelist YouTube/Vimeo, `rel="noopener noreferrer"` auf links, kein SVG/MathML. Keine exploitable Vulnerability gefunden. Re-Evaluate wenn sich das Rendering-Modell ändert (z.B. HTML-blob-rendering auf public).
...
- [x] **[Quick Win]** Read-only Filesystem (`read_only: true` + `tmpfs` für `/tmp`) — Sprint F 2026-04-19. `read_only: true` in beiden compose-Files, plus `tmpfs` mounts für `/tmp` + `/app/.next/cache` (Next.js interne fetch-cache + data-cache). Uploads landen in PostgreSQL BYTEA, kein FS-Write. `next/image` nicht genutzt (nativer `<img>`), so image-cache meist leer.
...
> **Verified 2026-04-19 (Sprint D1/E/F audit).** Alle 9 Anti-Patterns auditiert gegen aktuellen Codebase-State. Re-audit bei größeren Architektur-Änderungen (Rendering-Modell, Storage-Modell, Auth-Strategy).

- [x] **[Check]** Kein selbstgebautes Crypto irgendwo im Code — `jose` für JWT (HS256 pinned via `src/lib/jwt-algorithms.ts`), `bcryptjs` für Passwords (cost 12), `crypto.getRandomValues` für CSP-Nonces. Kein Hand-rolled-crypto.
- [x] **[Check]** Keine sensitive Daten in `localStorage` — Auth via HttpOnly Cookie (`__Host-session` in prod, `session` in dev). Dashboard-State via React state, nicht localStorage.
- [x] **[Check]** Keine sequenziellen IDs für public Resources — Media-URLs sind UUID-basiert (`/api/media/<uuid>/`), `media.public_id` als UUID separate von interner serial `id`. Projekte verwenden locale-aware slugs (`slug_de`/`slug_fr`), nicht IDs.
- [x] **[Check]** Email allein reicht NICHT als Identitäts-Beweis für Privileged Actions — Login requires password (bcrypt cost 12). Admin-mutations via session-cookie → `requireAuth` (`src/lib/api-helpers.ts`) → `verifySessionDualRead` (JWT verify). Password-change requires `current_password`-re-verify (`src/app/api/dashboard/account/route.ts`).
- [x] **[Check]** Kein Request-Body-Logging in Production — `auditLog()` loggt structured fields (ip, email, event-specific), NIEMALS raw body. Login-route loggt `login_success`/`login_failure` mit email only (kein password). `/api/csp-report` loggt normalized violation fields, nicht raw body.
- [x] **[Check]** CORS wird NICHT als Auth-Mechanismus verwendet — Auth via HttpOnly-Cookie + JWT + `sameSite: "lax"` (CSRF-Protection). CORS nicht konfiguriert für cross-origin auth, dashboard ist same-origin.
- [x] **[Check]** Kein `unsafe-inline` in CSP „weil wir kein nonce-Setup haben" — Sprint D1 live seit 2026-04-19: `script-src 'self' 'nonce-{N}' 'strict-dynamic'`. `style-src 'self' 'unsafe-inline'` bleibt pragmatisch wegen React-Inline-`style`-Props (nicht "weil kein nonce-setup", sondern strict-style-src ist separater Sprint mit Refactor-Aufwand).
- [x] **[Check]** Keine Secrets in Build-Output Env-Vars — Projekt nutzt `NEXT_PUBLIC_*` gar nicht. Alle secrets (JWT_SECRET, IP_HASH_SALT, ADMIN_PASSWORD_HASH, DATABASE_URL) als server-only env-vars via `process.env.X`. `SITE_URL` ist öffentlich, aber wird als regular env gelesen (nicht `NEXT_PUBLIC_`), um runtime-Override per docker-compose `environment:`-Block zu erlauben (siehe `src/lib/site-url.ts`).
- [x] **[Check]** Backups wurden mindestens einmal restored — Restore-Drill 2026-04-18: 13MB dump restored zu ephemeral `alit_restore_drill` DB in ~1s, alle 10 Tables + row-counts (admin_users=2, journal_entries=11, media=10, projekte=7 etc.) verifiziert, cleanup erfolgreich. Daily backup-cron active (`/opt/backups/alit-backup.sh` @ 03:00 UTC, 14d retention).
