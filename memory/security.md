...
# Last project update: 2026-04-19 — **T0 komplett + T1 zu 50%+ + T2 Docker-Block 7/8**. 14-PR-Session: Sprint D1 CSP Report-Only baseline live (PR #81 — CSP + `/api/csp-report/` + streaming-body-cap + legacy/modern-report-normalization; D2 enforced-flip wartet auf ≥7d clean stream ab 2026-04-19), Sprint E Docker-hardening (PR #82 — cap_drop + no-new-privileges + resource-limits), Sprint F read-only-FS (PR #88 — read_only:true + tmpfs für /tmp + /app/.next/cache), Trivy CI (PRs #86+#87 — filesystem+config+image scan mit `.trivyignore` VEX-whitelist für picomatch CVE-2026-33671), DOMPurify-Audit (audit-only, keine Migration — architektonische XSS-Mitigation via structured-blocks-rendering statt HTML-blob ist stark genug). Davor 2026-04-18: Ops-Follow-ups (Branch-Protection, Secret-Scanning, Backup-Cron, DB-REVOKE, `.env` chmod 600, JWT_SECRET+IP_HASH_SALT split staging↔prod). SSH + fail2ban + unattended-upgrades + UFW-bewusst-weg aus 2026-04-17/-18. **Verbleibend:** Tier-1 CSP D2-flip (date-gated), Sprint C Cookie-Migration Phase 2 (date-gated), Tier-2 Slim/Distroless-Base-Image (nominal offen, alpine ist schon slim), Zod (out-of-scope by design).
