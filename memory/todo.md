...
- [ ] **Dirty-Editor-Warnung bei Tab-Switch** (UX) — wenn User in Agenda/Journal/Projekte/Alit-Editor Änderungen macht und Top-Tab wechselt, soll Confirm-Modal "Ungesicherte Änderungen verwerfen?" erscheinen. Architektur vorgeschlagen: zentraler `DirtyContext` (Provider in `dashboard/page.tsx`) mit `setDirty(key, isDirty)` + `confirmDiscard(action)`; Tab-Buttons wrappen onClick; `beforeunload`-Listener für Browser-Close. Scope medium: Context + 4 Editor-Wirings. Eigener PR.
...
- [ ] [SEO] `src/app/robots.ts` mit `Sitemap:`-reference anlegen. Komplementiert sitemap.ts aus Sprint 5.
