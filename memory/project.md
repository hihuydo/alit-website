...
Last updated: 2026-04-18 (Mobile Dashboard Sprint B1 abgeschlossen — PR #74. `<ListRow>`-Primitive extrahiert aus Agenda/Journal/Projekte/Alit-Rows mit CSS-Dual-DOM responsive actions cluster — `hidden md:flex` horizontal buttons auf ≥md, `md:hidden` "…"-Menu auf <md mit Modal-reuse. Drag-Props explizit typed (draggable/onDragStart/Enter/Over/End/rowId). matchMedia-Listener für auto-close auf Desktop-resize. Single-modal-stack via action-invoked-after-setOpen(false). Badges bleibt opaque ReactNode — Projekte-`archiviert` inline in content. Bonus: HashtagEditor vertical-stack auf <md (user-spotted). 11 neue ListRow-Tests (248 total). Codex-Spec R1 7 findings v2 addressed; PR-R1 CLEAN. Davor: Dashboard-force-dynamic (PR #72, `export const dynamic` in layout.tsx), Mobile Dashboard Sprint A Foundations (PR #73, Modal mobile-first + MobileTabMenu Burger-Nav + safe-area-aware containers + DragHandle 44×44 + Login-Form), T0-Auth-Hardening Sprint B Cookie-Migration (PR #71). Nächster Sprint: Sprint B2 — SignupsSection Table→Card, RichTextEditor Toolbar, MediaSection 5-Button-Cluster, PaidHistoryModal, MediaPicker.)
