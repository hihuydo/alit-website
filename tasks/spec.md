...
3. **API-Roundtrip:** GET-Liste (`agenda/route.ts`) returnt `images_as_slider` automatisch (`SELECT *`-Pattern existiert bereits — nur die DDL muss laufen, kein Code-Change in der Route nötig); POST (`agenda/route.ts`) nimmt's an (Default false bei Omission), INSERT-Statement um `images_as_slider`-Spalte erweitert; PUT (`agenda/[id]/route.ts`) akzeptiert es als optionalen Field-Patch (Standard partial-PUT — kein nullable, daher kein CASE-WHEN nötig). **Kein GET-Handler in `[id]/route.ts`** — die Route hat aktuell nur PUT + DELETE, Dashboard liest Detail via List-GET-Iteration; kein neuer GET-Handler in diesem Sprint (wäre Scope-Creep ohne Caller). **Type-Surface in beiden Routes:** Der `parseBody<{...}>`-Generic (POST in `agenda/route.ts`, PUT in `agenda/[id]/route.ts`) bekommt `images_as_slider?: boolean` ergänzt — sonst `body.images_as_slider`-Access TS-Fail. **Field-Detection-Pattern: existing `destructuring + !== undefined` adoptieren** (NICHT `'in'`-Operator, NICHT `Object.hasOwn`). Die existing PUT-Route nutzt durchgängig `const { datum, ... } = body; if (datum !== undefined) { ... }` — `images_as_slider` läuft analog. **Boolean-Validierung im PUT** (und symmetrisch im POST falls Field present): `if (images_as_slider !== undefined && typeof images_as_slider !== 'boolean') return 400`. Eine Schreibweise pro Route, konsistent mit existing Code. **Audit:** Kein neues Audit-Event in diesem Sprint. Begründung: Die existing `agenda/[id]/route.ts` hat aktuell **keinen** `auditLog()`-Call (verifiziert) — „erweitern" hat nichts zu erweitern; comprehensive `agenda_update` Audit-Coverage ist eine eigenständige Diskussion (Scope, Schema, Pre-SELECT für Diff) und kein Slider-Concern. Als Follow-up in `memory/todo.md` loggen.
