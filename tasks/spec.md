...
1. **DB-Migration:** additive Spalte `agenda_items.images_as_slider BOOLEAN NOT NULL DEFAULT false` via `ALTER TABLE … ADD COLUMN IF NOT EXISTS …` in `ensureSchema()`. Idempotent, shared-DB-safe. **Compatibility-Contract bei staging-altered DB + prod-old-code:** (a) Old `SELECT *`-Reads (z.B. `agenda/route.ts:69-86`) tolerieren das extra-Field — wird im Output ignoriert, kein Crash; (b) Old PUT preserves unknown DB-Fields weil partial-PUT nur explizit gesetzte Felder updated; (c) Old POST INSERT lässt `images_as_slider` weg → DB-DEFAULT `false` greift, Insert succeeds. **Rollback-Note:** Falls neuer Code revertet werden muss nach DDL-Deploy, bleibt die Spalte mit DEFAULT false bestehen — alter Code läuft unbeschadet weiter, kein DROP COLUMN nötig (Spalte ist dormant). DROP wäre Phase-2-Sprint via 3-Phasen-Pattern (siehe `lessons.md` 2026-04-22 PR #106→#108).
2. **Dashboard-Toggle:** Checkbox „Bilder als Slider anzeigen (statt Grid)" im Agenda-Editor unterhalb der Bilder-Liste. Disabled (mit Hint-Text) wenn `< 2 Bilder` hochgeladen sind. Speichert via bestehender PUT/POST Roundtrip. **Edit-Open Form-Init (kritisch — sonst Silent-Data-Loss):** Beim Öffnen eines existing Items für Edit (in `openEdit(item)` oder vergleichbarem Handler in `AgendaSection.tsx`) muss `form.images_as_slider` aus `item.images_as_slider` befüllt werden — sonst wird der Form-State auf `false` (von `emptyForm`) initialisiert und jedes Save eines existing `slider=true`-Items setzt es silent zurück. Konkret: dort wo aktuell `images: (item.images ?? []).map(...)` (Z. 156) gemappt wird, parallel `images_as_slider: item.images_as_slider ?? false` setzen (Coalesce für Defense-in-Depth gegen alte API-Responses ohne Field). **Auto-Reset:** Wenn die Bildanzahl (durch Löschen) unter 2 fällt UND `images_as_slider===true` im Form-State steht, wird der Form-State auf `false` zurückgesetzt (in `removeImage`-Handler) — verhindert DB-Inkonsistenz „Toggle ON ohne Wirkung". **`previewItem` useMemo (Z. 219) MUSS `imagesAsSlider: form.images_as_slider` ins return-Object mappen** — sonst zeigt der Dashboard-Live-Preview (`<AgendaItemPreview item={previewItem} ... />`, Z. 639) immer das Grid statt Slider, auch wenn der Toggle ON ist. Da `imagesAsSlider?: boolean` im public Type optional ist, fängt TS die Omission NICHT — silent UX-regression sonst. **Type-Surface-Updates** (sonst TS-Fail bei `pnpm build`): (a) Dashboard-`AgendaItem`-Interface in `AgendaSection.tsx` (Z. 32–45) bekommt `images_as_slider: boolean`; (b) `emptyForm`-Constant (Z. 62–73) bekommt `images_as_slider: false`; (c) Form-State-Type bekommt `images_as_slider: boolean`. Audit-Trail: kein neues Audit-Event in diesem Sprint (siehe Req #3 Note).
3. **API-Roundtrip:** GET-Liste (`agenda/route.ts`) returnt `images_as_slider` automatisch (`SELECT *`-Pattern existiert bereits — nur die DDL muss laufen, kein Code-Change in der Route nötig); POST (`agenda/route.ts`) nimmt's an, **INSERT-Statement explizit `images_as_slider` als Spalte UND Wert: `INSERT INTO agenda_items (..., images_as_slider) VALUES (..., $N)` mit `body.images_as_slider ?? false` als Parameter** (NICHT auf DB-DEFAULT verlassen — sonst silent-ignore von `true` aus dem Form, Create-Flow würde nie `slider=true` schreiben); PUT (`agenda/[id]/route.ts`) akzeptiert es als optionalen Field-Patch (Standard partial-PUT — kein nullable, daher kein CASE-WHEN nötig). **Kein GET-Handler in `[id]/route.ts`** — die Route hat aktuell nur PUT + DELETE, Dashboard liest Detail via List-GET-Iteration; kein neuer GET-Handler in diesem Sprint (wäre Scope-Creep ohne Caller). **Type-Surface in beiden Routes:** Der `parseBody<{...}>`-Generic (POST in `agenda/route.ts`, PUT in `agenda/[id]/route.ts`) bekommt `images_as_slider?: boolean` ergänzt — sonst `body.images_as_slider`-Access TS-Fail. **Field-Detection-Pattern: existing `destructuring + !== undefined` adoptieren** (NICHT `'in'`-Operator, NICHT `Object.hasOwn`). Die existing PUT-Route nutzt durchgängig `const { datum, ... } = body; if (datum !== undefined) { ... }` — `images_as_slider` läuft analog. **Boolean-Validierung in BEIDEN Routes (POST + PUT) explizit, nicht parenthetisch**: `if (images_as_slider !== undefined && typeof images_as_slider !== 'boolean') return 400` — derselbe Guard in `agenda/route.ts` (POST) und `agenda/[id]/route.ts` (PUT). Eine Schreibweise pro Route, konsistent mit existing Code. **Audit:** Kein neues Audit-Event in diesem Sprint. Begründung: Die existing `agenda/[id]/route.ts` hat aktuell **keinen** `auditLog()`-Call (verifiziert) — „erweitern" hat nichts zu erweitern; comprehensive `agenda_update` Audit-Coverage ist eine eigenständige Diskussion (Scope, Schema, Pre-SELECT für Diff) und kein Slider-Concern. Als Follow-up in `memory/todo.md` loggen.
...
5. **Frontend-Slider-Component:** Neuer File `src/components/AgendaImageSlider.tsx` (Client Component). CSS scroll-snap horizontal, `scroll-snap-type: x mandatory`, jedes Bild `scroll-snap-align: center` + **`scroll-snap-stop: always`** (verhindert dass schnelle Touch-Swipes mehrere Slides überspringen). **Full-Panel-Width via `width: 100%` ohne side-padding-Wrapper** — `AgendaItem.tsx` hat selbst kein side-padding (Padding wird per-child gesetzt), also reicht es, den Slider OHNE den `padding: 0 var(--spacing-base) var(--spacing-base)`-Wrapper zu rendern, dann nimmt er automatisch die volle AgendaItem-Breite = volle Panel-Breite. **Negative-Margin / `100vw + translateX(-50%)` werden NICHT verwendet** — der Parent `<div className="overflow-hidden">` (Accordion-Animation-Wrapper, Z. 164) clippt visuell alles außerhalb seiner content-box, und `100vw` würde über die Panel-Grenze ins Nachbarpanel hinausragen. Fixe Slider-Höhe via inline-style `height: clamp(240px, 30vw, 420px)` (Werte in Phase 3 am Layout feinjustiert). **Slide-Wrapper:** jeder Slide ist ein `<div>` mit **stabilem `key={img.public_id}`** (Fallback `key={i}` falls public_id nicht eindeutig — react/jsx-key-Pflicht), Klassen `flex items-center justify-center shrink-0 h-full` mit `style={{ scrollSnapAlign: "center", scrollSnapStop: "always" }}` (oder Tailwind `snap-center snap-always`). `flex-shrink: 0` ist **zwingend** — Default `flex-shrink: 1` würde alle Slides zu gleichmäßig 1/N-Container-Breite stauchen und die Aspect-Ratio-Sizing zerstören. Bilder im Slider mit `height: 100%; width: auto; object-fit: contain` → Breite = Höhe × native Aspect-Ratio, horizontal zentriert per Slide-Wrapper. **CLS-Prevention:** Jedes `<img>` braucht HTML-Attribute `width={img.width ?? (img.orientation === "portrait" ? 3 : 4)}` und `height={img.height ?? (img.orientation === "portrait" ? 4 : 3)}` (Orientation-basierter Fallback für Legacy-Rows mit `null`-Dimensions, identisch zu `AgendaItem.tsx:174-184` Grid-Renderer). Sonst CLS und kollabierte Slides bei Legacy-Bildern. **Lazy-Loading:** `loading="lazy"` auf jedem `<img>` — konsistent mit Grid-Renderer + Out-of-Scope-Statement. **`alt={img.alt ?? ""}`** auf jedem `<img>`. **Scroll-Container-Klassen (vollständig):** `flex overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden` plus `style={{ scrollSnapType: "x mandatory" }}` (oder Tailwind `snap-x snap-mandatory`). **`overflow-x-auto` ist zwingend** — ohne overflow-context ist `scroll-snap-type` ein no-op und der Slider scrollt gar nicht. `flex` ist zwingend für die `flex-shrink: 0` Slide-Wrapper-Mechanik. Cross-Browser-Scrollbar-Hiding via Arbitrary-Values konsistent mit `tailwind.md`-Pattern und RichTextEditor-Toolbar aus PR #78. **SSR-Safety:** Alle browser-only APIs (`IntersectionObserver`, `Element.scrollIntoView`, `window.matchMedia`) MÜSSEN in `useEffect(() => {...}, [...])`-Bodies leben — nie im Component-Render-Body, nie auf Module-Level. Next.js App Router rendert auch `"use client"` Components SSR-seitig im ersten HTML-Pass, sonst `ReferenceError` zur Request-Time. **`useEffect`-Body-Beginn**: `if (!containerRef.current) return;` als erste Zeile vor IO-Setup (defensive — JSDOM-Tests könnten ohne mounted Container rendern; `root: null` würde implicit-viewport setzen und falsches Verhalten geben).
6. **Dots-Indikator:** N Buttons unterhalb des Sliders, 1 Dot pro Bild. **Locale-aware Labels (Pflicht — Panel 1 ist locale-scoped via `getAgendaItems(locale)`):** Slider-Strings (`navLabel`, `dotLabel(i, n)`) kommen via Props vom Parent `AgendaItem.tsx`. AgendaItem ist bereits Client Component mit `useParams<{ locale: string }>()` (Z. 82) → ruft `getDictionary(locale as Locale)` aus `@/i18n/dictionaries` und passt `dict.slider_nav` + `dict.slider_dot` an Slider weiter. KEINE hardcoded German-Strings im Slider-Component (Architecture-Pattern: `AgendaImageSlider` bleibt presentational, locale-frei). Konkret: Props-Interface `{ images: AgendaImage[], navLabel: string, dotLabel: (i: number, total: number) => string }`. Dictionary-Einträge in `src/i18n/dictionaries.ts`: DE `{ slider_nav: "Bilder-Navigation", slider_dot: (i, n) => `Bild ${i+1} von ${n} anzeigen` }`, FR `{ slider_nav: "Navigation des images", slider_dot: (i, n) => `Afficher l'image ${i+1} sur ${n}` }`. **AgendaPanel.tsx braucht KEINEN dict-prop-Threading** — AgendaItem deriviert dict selbst aus `useParams`-locale. **Semantischer Wrapper:** `<nav aria-label={navLabel} className="flex justify-center gap-1">` (oder `<div role="group" aria-label={navLabel}>`) — minimum WCAG 1.3.6 Grouping. **Container-Ref + IO-root:** `const containerRef = useRef<HTMLDivElement>(null)` wird auf den Scroll-Container gesetzt. **Active-Slide-Detection: Center-Band-Algorithmus statt naivem `threshold: 0.5`** — Bei variabel-breiten Slides (Portrait + Portrait + Portrait) können mehrere Slides gleichzeitig >50% sichtbar sein → callback-order-dependent flicker. Lösung: `rootMargin: "0px -50% 0px -50%"` reduziert das IO-„root" auf eine 0px-breite Center-Linie; nur der Slide, dessen Center diese Linie kreuzt, gilt als active. `threshold: 0` reicht damit, da der „root" bereits eine Linie ist. IO im `useEffect`: `new IntersectionObserver(callback, { root: containerRef.current, rootMargin: "0px -50% 0px -50%", threshold: 0 })` — **`root` MUSS der Scroll-Container sein**, nicht null/viewport (sonst falsche Active-Detection wenn Panel teilweise scrolled). **Stabile Slide-Refs via `slidesRef = useRef<HTMLDivElement[]>([])`** (kein State-Array, vermeidet `useCallback`-deps-Drift / stale-closure-Trap aus `lessons.md` 2026-04-22 PR #110). **Active-State:** `const [activeSlide, setActiveSlide] = useState(0)` — initial 0 ist safe vs Hydration-Mismatch, weil `useReducedMotion` (Req #9) als `useSyncExternalStore` SSR=client=0 garantiert; IO feuert erst nach Mount und kann `activeSlide` ohne Render-Reconciliation-Fehler aktualisieren. **IO-Callback + observe-Loop (vollständige useEffect-Body):**
    ```ts
    useEffect(() => {
      if (!containerRef.current) return;
      slidesRef.current.length = images.length;  // trim stale refs nach image-count-drop (sonst observed IO detached DOM-Nodes)
      setActiveSlide(0);                          // reset out-of-bounds bei image-count-drop
      const observer = new IntersectionObserver(
        ...
        // Center-Band-Algorithmus statt naivem threshold: rootMargin reduziert
        // den IO-„root" auf eine 0px-breite Center-Linie; nur der Slide dessen
        // Center diese Linie kreuzt zählt als active. Verhindert flicker bei
        // mehreren gleichzeitig >50%-sichtbaren narrow Portrait-Slides.
        { root: containerRef.current, rootMargin: "0px -50% 0px -50%", threshold: 0 }
      );
      for (const slide of slidesRef.current) {
        if (slide) observer.observe(slide);  // each slide MUSS observed werden
      }
      return () => observer.disconnect();
    }, [images]);  // re-bind bei JEDER images-Mutation — auch Reorder mit gleicher Länge erzeugt neue DOM-Elemente via key-Änderung; [images.length] würde Reorder verpassen → IO observed detached old elements, Dot-Indicator freezes
    ```
    `observe()`-Loop fehlend = IO feuert nie = Dots stuck auf Slide 0. Klick-Handler: `(i: number) => slidesRef.current[i]?.scrollIntoView({ behavior: prefersReducedMotion ? 'auto' : 'smooth', inline: 'center', block: 'nearest' })` — `prefersReducedMotion` kommt aus dem in Req #9 spezifizierten Hook. Buttons mit `aria-label="Bild N anzeigen"` + `aria-current="true"` auf aktivem Dot. **Touch-Target ≥28×28 konkret:** Dot-Visual `w-2 h-2` (8px), Button-Padding `p-3` (12px), ergibt 32×32 Touch-Target ≥ WCAG 2.5.5. **Aktiv-Visual via Conditional className** (NICHT Tailwind `aria-current:` Variant — die ist self-referential und matcht nur das Element das `aria-current` selbst trägt; `aria-current` ist auf `<button>`, der Dot-Span ist Child → würde nie aktivieren): `<button className="p-3 inline-flex items-center justify-center"><span className={`block w-2 h-2 rounded-full bg-current ${activeSlide === i ? "opacity-100" : "opacity-50"}`} /></button>`.
7. **Renderer-Branching:** `AgendaItem.tsx:165-189` — der existing `{images.length > 0 && (<div className="grid grid-cols-2 ...">...</div>)}`-Block wird durch ein **single conditional ternary ERSETZT** (NICHT als sibling addiert — sonst rendern Slider UND Grid gleichzeitig). Konkret:
   ```tsx
   {images.length > 0 && (
     images.length >= 2 && item.imagesAsSlider === true
       ? <AgendaImageSlider images={images} navLabel={dict.slider_nav} dotLabel={dict.slider_dot} />
       : <div className="grid grid-cols-2 gap-[var(--spacing-half)]" style={{ padding: "0 var(--spacing-base) var(--spacing-base)" }}>{/* existing grid map */}</div>
   )}
   ```
   Der äußere `images.length > 0`-Guard bleibt (kein Block bei leerer Liste). Bei 1 Bild oder Toggle OFF → bisheriges Grid bit-identisch. Bei 2+ Bilder + Toggle ON → Slider OHNE Side-Padding-Wrapper.
...
9. **Reduced-Motion:** Single Implementation via neuem `useReducedMotion()` Hook in `src/lib/use-reduced-motion.ts`. **Pflicht-Pattern: `useSyncExternalStore`** (siehe `patterns/react.md` — `window.matchMedia`-Reads sind explizit als `useSyncExternalStore`-Case gelistet, NICHT `useState + useEffect`). **Subscribe + Snapshot Module-level hoisten** (sonst re-subscribe-Spam: jeder Render im Consumer erzeugt neue Function-References → useSyncExternalStore tear-down + re-subscribe der matchMedia-Listener pro Render). Implementation:
    ```ts
    // Module-level: ein einziger MQL-Object über alle Reads geteilt
    // (sonst neue MQL-Allokation pro getSnapshot-Call → useSyncExternalStore
    // ruft getSnapshot oft pro Render auf = unnötiger GC-Druck).
    // Lazy initialisiert weil window auf Server undefined.
...
10. **Tests:**
    ...
    - Branching-Test `src/components/AgendaItem.test.tsx` **neu erstellen** (existiert nicht): (a) `imagesAsSlider=true, 3 Bilder` → `<AgendaImageSlider>` gerendert (Mock auf den Slider-Component, assert Mock-Call), kein Grid; (b) `imagesAsSlider=true, 1 Bild` → Grid (kein Slider-Mock-Call); (c) `imagesAsSlider=false, 3 Bilder` → Grid. **Render mit `defaultExpanded={true}`** — sonst ist der Accordion-Body collapsed (`grid-rows-[0fr]` + `inert`) und alle Slider/Grid-Assertions sind vacuous oder werfen „not found". **Export-Form-Konvention:** `AgendaImageSlider` MUSS **named export** sein (`export function AgendaImageSlider(...)`) — konsistent mit `AgendaItem`/`JournalSidebar`/`AgendaPanel`-Pattern in `src/components/`. **Test-Setup** entsprechend: `vi.mock("next/navigation", () => ({ useParams: () => ({ locale: "de" }) }))` UND `vi.mock("@/components/AgendaImageSlider", () => ({ AgendaImageSlider: vi.fn(() => <div data-testid="slider-mock" />) }))` — der Mock-Key MUSS dem named export matchen, sonst greift der Mock nicht und Tests rendern den echten Slider statt des Mocks.
...
| `src/app/api/dashboard/agenda/[id]/route.ts` | Modify | PUT akzeptiert optional `images_as_slider`, Boolean-Type-Validation 400, UPDATE setzt nur wenn übermittelt (destructuring + `!== undefined`-Pattern, exakt wie existing `datum`/`zeit`-Felder). **Kein neuer GET-Handler** — Route hat aktuell nur PUT + DELETE. **Kein Audit-Call** — existing Route hat keinen `auditLog()`, comprehensive Audit-Coverage als Follow-up. |
...
- Shared-DB-Pattern: Staging-Push triggert ALTER auf prod-shared-DB. Compatibility-Contract bei prod-old-code in der Übergangsfenster-Periode: (a) `SELECT *`-Reads in der dashboard list route (`agenda/route.ts:69-86`) tolerieren extra Felder — keine Output-Crash; (b) old PUT ist partial → unknown DB-Fields bleiben unverändert; (c) old POST lässt das neue Field weg → DB-DEFAULT `false` greift; (d) Public reader `getAgendaItems()` ist explizit-SELECT (kein `SELECT *`) und braucht eine Route-Update bevor er `imagesAsSlider` zurückgibt. Insgesamt safe-by-default. Siehe `patterns/deployment-staging.md` und `lessons.md` 2026-04-22 PR #106→#108-Pattern für additive Migrations — DROP braucht 3 Phasen, ADD nicht.
