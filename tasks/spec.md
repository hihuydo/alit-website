...
<!-- R8 (2026-05-01): User PR-style review caught 4 echte Findings post-Codex: (P1) DK-11 smoke-test referenziert noch newsletter.intro trotz DK-3-Drop, (P1) DirtyKey-Union/DIRTY_KEYS/INITIAL_DIRTY brauchen submission-texts-Erweiterung (sonst Build-Fail oder Outer-Tab-Guard nicht registered), (P1) Etag-Format-Inkonsistenz GET-ISO vs DB updated_at::text bei TIMESTAMPTZ, (P2) Editor initial display muss raw GET mit dict-defaults mergen für display (sonst leere Felder bei first-open). Plus Drift in DK-4 (`newsletter: 8 fields` → 7). Alle inline gefixt. -->
...
- **DK-6** Neuer Editor-Component `SubmissionTextsEditor.tsx` im `src/app/dashboard/components/`. Pattern strikt analog `JournalInfoEditor.tsx`:
  ...
  - **Initial Display Merge** (R8 fix für leere-Felder-bei-first-open): Editor's GET-Response ist raw-normalized (DK-1, partial structure ohne Defaults). Editor MUSS für die UI-Anzeige die raw-data mit `getDictionary(locale)`-Defaults mergen via `pickEditableFields`-helper, BEFORE der initialen `setState`. Konkret im GET-`useEffect`: `const display = mergeWithDefaults(serverData, getDictionary)` wo merge-rule per-Field: `display[form][locale][field] = serverData[form][locale][field] || dictionary[locale][form][field]`. Snapshot dann `JSON.stringify(display)`. **Save-Pfad** sendet weiterhin nur die nicht-Default-Werte zurück — sonst würde das Save-PUT die DB mit dictionary-defaults füllen, was die per-Field-Fallback-Logik in DK-4 bricht. Konkret: vor PUT, strip Felder die exact mit dictionary übereinstimmen → `payload[form][locale][field]` is omitted if `=== dictionary-default`. Damit bleibt DB-Storage minimal + per-Field-Fallback weiterhin live.
...
- **DK-10** Test-Coverage:
  ...
  - `submission-form-texts/route.test.ts`:
    - GET (leer-DB → defaults für beide Forms × Locales, etag null)
    - GET (gesetzt → returns nested JSON, etag === updated_at-ISO)
