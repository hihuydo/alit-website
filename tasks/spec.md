...
9. **Reduced-Motion:** Single Implementation via neuem `useReducedMotion()` Hook in `src/lib/use-reduced-motion.ts`: SSR-safe (`typeof window === 'undefined' → false`), `window.matchMedia('(prefers-reduced-motion: reduce)').matches`, `addEventListener('change', ...)` + cleanup für Live-Updates. Slider importiert + nutzt das im `prefersReducedMotion`-State (Req #6). **Kein CSS-only-Fallback** — die JS-side `scrollIntoView({behavior:'smooth'})` auf Dot-Klick wird durch CSS `@media`-Query NICHT abgedeckt (WCAG 2.3.3 violation), daher muss der Hook autoritativ sein. Zusätzlich CSS-Schutz im scroll-container `@media (prefers-reduced-motion: reduce) { scroll-behavior: auto }` als defense-in-depth für native Touch-Swipe.
10. **Tests:**
    ...
    - Branching-Test `src/components/AgendaItem.test.tsx` **neu erstellen** (existiert nicht): (a) `imagesAsSlider=true, 3 Bilder` → `<AgendaImageSlider>` gerendert (Mock auf den Slider-Component, assert Mock-Call), kein Grid; (b) `imagesAsSlider=true, 1 Bild` → Grid (kein Slider-Mock-Call); (c) `imagesAsSlider=false, 3 Bilder` → Grid. **Test-Setup**: `vi.mock("next/navigation", () => ({ useParams: () => ({ locale: "de" }) }))` UND `vi.mock("@/components/AgendaImageSlider", () => ({ default: vi.fn(() => <div data-testid="slider-mock" />) }))` — letzteres isoliert AgendaItem-Branching-Logic vom Slider-Internal.
    ...
    - API-Route-Test `agenda/[id]/route.test.ts` erweitert: (1) GET returnt Feld; (2) PUT updated Feld; (3) PUT mit `images_as_slider: "true"` (String statt Boolean) → 400 Boolean-Validation-Test; (4) POST mit Default false bei Omission.
11. **Verification:**
