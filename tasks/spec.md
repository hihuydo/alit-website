...
7. **Dashboard Grid-first UX (Single-Image-Mode + Grid-Modes)** — `AgendaSection.tsx` Image-Block wird komplett neu gebaut:
   - **Mode-Picker** ganz oben (immer sichtbar): „Einzelbild" (`cols=1`) | „2 Spalten" | „3 Spalten" | „4 Spalten" | „5 Spalten". Persistiert als `images_grid_columns`. Default neuer Eintrag: `1` (Einzelbild). **Alle Mode-Picker- und Fit-Toggle-Buttons MÜSSEN explizit `type="button"` haben** (innerhalb der bestehenden `<form>`-Struktur in AgendaSection — sonst default-`type="submit"` → Click triggert PUT/POST + closes Editor (patterns/react.md: Form-Submit-Trap). Gleiches gilt für „+ neue Zeile"-Button + ✕-Remove + Crop-Button. Test: prüfen `getAttribute("type") === "button"` für jeden neuen Button.
   ...
   - **Inline-Hint Lifecycle**: hint-State ist transient pro Slot, gespeichert als z.B. `Record<number, string | null>` State im AgendaSection. Hint wird gecleart bei: nächstem Drop-Attempt auf denselben Slot, ✕-Remove des Slots (= Slot wieder empty), Mode-Wechsel (Reset des gesamten hint-State), Editor close+reopen. Verhindert dass nach Failure→Retry-Success sowohl Thumbnail als auch Failure-Hint sichtbar sind.
...
12. **Tests grün** — `pnpm build` + `pnpm test` + `pnpm exec tsc --noEmit` clean. Mindestens **+38 neue Tests** für:
    ...
    - `AgendaSection.test.tsx` (extend) — Mode-Picker-Render mit allen 5 Optionen, Mode-Picker-Buttons haben `type="button"`-Attribut, Mode-Wechsel preserves Bilder + resettet visibleSlotCount, neuer Eintrag (emptyForm) zeigt 1 empty slot (cols=1, visibleSlotCount=1), openEdit mit `images: null` rendert ohne Crash (null-Guard), „+ neue Zeile" erhöht visibleSlotCount um cols, „+ neue Zeile" Button bei cols=1 ist `disabled` (existiert in DOM, nicht hidden), Empty-Slot click triggert MediaPicker mit korrektem Target-Index, Empty-Slot click → onClose-trigger ohne Auswahl → assert pickerTargetSlot=null, OS-File-Drop auf empty Slot triggert Upload, OS-File-Drop auf filled Slot ist Noop, Multi-File-Drop läuft sequentiell, Multi-File-Drop bei Failure von File 2: File 1 bleibt persisted (kein Rollback), Single-File-Upload-Failure: kein partial-append + Inline-Hint sichtbar, Hint clears nach Retry-Success, Drag-Reorder same-row swap insert-before mit exakter post-array-Assertion, Drag-Reorder cross-row, Drag-Reorder onto-empty-slot-beyond-filled = append, Soft-Warning „Letzte Reihe..." erscheint/verschwindet, Soft-Warning „Einzelbild..." (aus i18n.tsx, nicht hardcoded) erscheint bei cols=1+length>=2, cols=1 + 3 images zeigt 3 filled slots im Editor (Slot-Grid versteckt nichts), Mode-Wechsel updated `previewItem.imagesGridColumns`, slot-element hat `onDragOver` handler im DOM, Slot-Grid nutzt inline `style.gridTemplateColumns` (nicht Tailwind-arbitrary).
...
- **HTML5 DragEvent Type-Discrimination Protocol** — Slot-`onDrop` muss zwei Drop-Sources auseinanderhalten (filled-slot-Reorder vs OS-File-Upload):
  - **`onDragOver={e => e.preventDefault()}` PFLICHT auf JEDEM Slot** (empty + filled). Ohne `preventDefault` setzt der Browser `dropEffect = 'none'` und `onDrop` wird nie dispatched — feature ist silent broken in Production während Vitest-Mocks (`fireEvent.drop`) trotzdem grün sind.
  ...
  - In `onDrop`: erst `e.dataTransfer.types.includes('Files')` prüfen → OS-File-Upload-Branch (für empty slot) oder Noop (für filled slot). Sonst `e.dataTransfer.getData('text/slot-index')` lesen → Reorder-Branch (insert-before, Splice-Pattern unten).
