...
7. **Dashboard Grid-first UX (Single-Image-Mode + Grid-Modes)** — `AgendaSection.tsx` Image-Block wird komplett neu gebaut:
   ...
   - **`openEdit`-Mapping initialisiert `visibleSlotCount = form.images_grid_columns`** (nur cols, NICHT `Math.max(cols, images.length)`). **`emptyForm` initialisiert `visibleSlotCount = 1`** (= default cols). Display-Formel `Math.max(visibleSlotCount, images.length)` zeigt eh alle Bilder (Slot-Grid versteckt nie ein bestehendes Bild). Klick auf „+ Zeile" gibt deterministisch `cols + cols` Slots, nicht `max(...) + cols`.
   ...
   - **MediaPicker `onClose` callback MUSS via `useCallback`** stabilisiert sein (lessons.md 2026-04-19, patterns/react.md „Modal-Parent callback stability"). Sonst Focus-Reset bei jedem Re-Render während Picker offen ist. Gleiches gilt für `onSelect` (= `handleMediaSelect`), das `pickerTargetSlot` State liest. **`onClose` resettet zusätzlich `pickerTargetSlot=null`** (sonst stale-state: User klickt Slot 3, schließt ohne Auswahl, klickt Slot 1 — Picker liest stale `pickerTargetSlot=3`). Test-Coverage: Empty-Slot click → onClose-trigger → assert `pickerTargetSlot === null`.
...
12. **Tests grün** — `pnpm build` + `pnpm test` + `pnpm exec tsc --noEmit` clean. Mindestens **+34 neue Tests** für:
    ...
    - `AgendaSection.test.tsx` (extend) — Mode-Picker-Render mit allen 5 Optionen, Mode-Wechsel preserves Bilder + resettet visibleSlotCount, neuer Eintrag (emptyForm) zeigt 1 empty slot (cols=1, visibleSlotCount=1), „+ neue Zeile" erhöht visibleSlotCount um cols, „+ neue Zeile" Button bei cols=1 ist `disabled` (existiert in DOM, nicht hidden), Empty-Slot click triggert MediaPicker mit korrektem Target-Index, Empty-Slot click → onClose-trigger ohne Auswahl → assert pickerTargetSlot=null, OS-File-Drop auf empty Slot triggert Upload, OS-File-Drop auf filled Slot ist Noop, Multi-File-Drop läuft sequentiell, Multi-File-Drop bei Failure von File 2: File 1 bleibt persisted, Single-File-Upload-Failure: slot reverts to empty + kein crash, Drag-Reorder same-row swap insert-before mit exakter post-array-Assertion, Drag-Reorder cross-row, Drag-Reorder onto-empty-slot-beyond-filled = append, Soft-Warning „Letzte Reihe..." erscheint/verschwindet, Soft-Warning „Einzelbild..." erscheint bei cols=1+length>=2, cols=1 + 3 images zeigt 3 filled slots im Editor (Slot-Grid versteckt nichts), Mode-Wechsel updated `previewItem.imagesGridColumns`, slot-element hat `onDragOver` handler im DOM.
