...
12. **Tests grün** — `pnpm build` + `pnpm test` + `pnpm exec tsc --noEmit` clean. Mindestens **+46 neue Tests** für:
    ...
    - `queries.test.ts` (extend oder Create) — `getAgendaItems()` Mapping-Tests: Row mit `images_grid_columns=3, images_fit="contain"` → AgendaItemData hat `imagesGridColumns: 3, imagesFit: "contain"`. Legacy-Row ohne Werte → AgendaItemData hat `imagesGridColumns: 1, imagesFit: "cover"` (defensive Fallback `?? 1` und `=== "contain" ? "contain" : "cover"`). Pflicht — sonst silent Public-Render-Regression wenn neue Spalten beim SELECT vergessen werden.
...
| `src/lib/queries.test.ts` | Create oder Extend | `getAgendaItems()` Mapping-Test: Row mit beiden Werten → korrekt durchgereicht. Legacy-Row → defensive Defaults. Verhindert silent Public-Read-Regression. |
...
| `src/app/dashboard/components/MediaPicker.tsx` | Modify | NUR Props-Interface erhält `targetSlot?: number \| null` als ignored passthrough-prop (wird intern NICHT verwendet, nur damit AgendaSection es type-safe durchreichen kann + Tests via Mock asserten können). KEIN Upload-Helper hier — MediaPicker bleibt rein für Selection-aus-Library. |
| `src/app/dashboard/components/agendaUpload.ts` (NEU, optional) ODER co-located in AgendaSection.tsx | Create | `uploadFileForAgenda(file: File): Promise<AgendaImage>` Helper. Implementation: extracted from `AgendaSection.tsx:251-293` (POST `/api/dashboard/media/` + `Image()`-probe für orientation/width/height). Return-Shape match `AgendaImage` direkt. Optional eigenes File für Test-Isolation; sonst als unexported function in AgendaSection.tsx. |
...
- **Drop-from-OS reuses bestehende AgendaSection-Upload-Pipeline programmatisch** — der existierende Upload-Flow in `AgendaSection.tsx:251-293` (POST `/api/dashboard/media/`, dann `Image()`-probe für `orientation/width/height`) wird in einen Helper extrahiert. **Falsche Annahme der vorigen Spec-Versionen**: `MediaPickerResult` ist der Rich-Text-Embed-Vertrag (`{type, src, caption, width}`), NICHT der AgendaImage-Vertrag (`{public_id, orientation, width, height}`). MediaPicker ist nur für Auswahl bestehender Medien, nicht für Upload im Agenda-Kontext. Korrekter Helper: `uploadFileForAgenda(file: File): Promise<AgendaImage>` (Agenda-shape direkt). Lokation: neuer `src/app/dashboard/components/agendaUpload.ts` ODER co-located helper in `AgendaSection.tsx` (kleinerer Footprint, kein neues File). MediaPicker.tsx bleibt dadurch komplett unangetastet.
- **Multi-File OS-Drop läuft sequentiell** — bei Drop von 2+ Files gleichzeitig: `let slotCursor = images.length; for (const file of files) { try { const r = await uploadFileForAgenda(file); setForm(f => append(f, r)); slotCursor += 1; } catch (err) { setSlotErrors(prev => ({ ...prev, [slotCursor]: t.agenda.uploadFailed })); console.error(err); break; } }`. KEIN concurrent `Promise.all` — sonst race condition. **`slotCursor` Pattern für error-index**: bei Multi-File mit Failure von File N gehen Files 1..N-1 in slots `images.length..images.length+N-2`, Failure-Hint geht auf `slots[images.length+N-1]` — sonst ist nicht deterministisch welcher Slot den Hint zeigt. Test-Coverage: 2-File-Drop, File 2 fails → assert File 1 in `images[L]`, slotErrors[L+1] gesetzt.
