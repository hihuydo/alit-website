...
- [ ] **DK-56** IO-Active-Detection: Center-Band-Algorithmus statt threshold:0.5. `new IntersectionObserver(cb, { root: containerRef.current, rootMargin: "0px -50% 0px -50%", threshold: 0 })` — verhindert flicker bei mehreren gleichzeitig >50%-sichtbaren narrow Slides. **Code-Block in Spec ist authoritative** — bei Inkonsistenz Text vs Code, Code-Block gilt
- [ ] **DK-57** Dictionary-Pfad korrekt: `src/i18n/dictionaries.ts` (NICHT `src/lib/dict.ts` — existiert nicht). DE/FR slider_nav + slider_dot(i, n) hinzufügen
- [ ] **DK-58** AgendaItem.tsx ruft `getDictionary(locale as Locale)` aus `@/i18n/dictionaries` direkt auf (nutzt existing `useParams<{locale}>()` Z. 82) und passt `dict.slider_nav` + `dict.slider_dot` als Props an Slider — KEINE prop-threading via AgendaPanel
