...
<!-- R8 (2026-05-01): User PR-style review caught 4 (P1 DK-11 smoke-test referenziert noch newsletter.intro, P1 DirtyKey-Union/DIRTY_KEYS/INITIAL_DIRTY brauchen Erweiterung + Test, P1 Etag-Format-Inkonsistenz GET-ISO vs DB::text, P2 Editor initial display merge mit dict für non-empty UI). Plus DK-4 drift (8→7). Alle gefixt. -->
...
- [ ] **DK-1** Neue API-Route `/api/dashboard/site-settings/submission-form-texts/` (GET auth-only, PUT auth+CSRF mit explizitem `pool.connect()` + `BEGIN/SELECT FOR UPDATE/UPSERT/COMMIT`). PUT-Body via `parseBody<T>(req)`. Zod `.strict()`-validation: outer body `{data, etag}`, BEIDE top-level form-keys + BEIDE locales required. **Etag canonical format = JS `Date.toISOString()`** (z.B. `"2026-05-01T13:42:08.123Z"`) — NICHT direkt PostgreSQL `updated_at::text` (`"2026-05-01 13:42:08.123+00"` ist ein anderes Format → false 409). GET typed Date → `.toISOString()`. **GET: `{data, etag: ISO|null}`**. **PUT request: `{data, etag}`**. **PUT 200: `{data, etag}`**. **PUT 409 stale_etag**: canonical-ISO compare nach SELECT FOR UPDATE, mismatch → ROLLBACK + 409. First-save-edge: DB-row missing AND body.etag null → success.
...
- [ ] **DK-6** Neuer Editor `SubmissionTextsEditor.tsx` analog `JournalInfoEditor.tsx`. Outer-Toggle Mitgliedschaft/Newsletter, Inner-Toggle DE/FR. `<input>` single-line, `<textarea>` für intro/successBody/privacy. **`isDirty` mit snapshotVersion-state-bump bei jeder ref-mutation**. **`userTouchedRef`-Guard + Reset-Button setzt ihn true**. **Re-snapshot vom Server-Response nach Save**. **DirtyContext `setDirty("submission-texts", isDirty)` + cleanup**. **`DirtyContext.tsx` MUSS erweitert werden**: `DirtyKey`-Union + `DIRTY_KEYS` array + `INITIAL_DIRTY` record alle um `"submission-texts"` (sonst TS-fail/unregistered guard). **`onDirtyChange?` callback prop** für SignupsSection. Defaults via `getDictionary`. **Initial Display Merge**: GET-Response (raw) + dict-defaults via `pickEditableFields` mergen für UI-Display, snapshot vom merged-state; Save strip't dict-equal Felder vor PUT damit per-Field-Fallback (DK-4) live bleibt. **Etag-State + 409-Handling**: `currentEtag` state, PUT sendet `{data, etag}`, on 200 → setCurrentEtag, on 409 → `staleConflict` banner + `[Neu laden]` button.
...
- [ ] **DK-11** Manueller Visual-Smoke: Default-Werte stimmen mit dictionary überein (initial-merge funzt), Mitgliedschaft-DE-heading-change reflected on /mitgliedschaft, Newsletter-FR-`privacy`-change (NICHT `intro`! das ist out-of-scope DK-3) reflected auf `/projekte/discours-agites`, Reset-to-Default funktioniert, Logout-during-dirty kein Crash.
