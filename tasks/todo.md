...
<!-- Spec: tasks/spec.md -->
<!-- Started: 2026-04-18 -->
<!-- Status: Draft v1 — awaiting approval + post-commit Sonnet-Evaluator -->
...
- [ ] `src/app/dashboard/components/ListRow.tsx` existiert mit Props `dragHandle?`, `content`, `badges?`, `actions: RowAction[]`.
- [ ] `ListRow` rendert auf ≥md horizontale Action-Buttons, auf <md ein "…"-Button + `RowActionsMenu`-Modal.
...
- [ ] `ListRow.test.tsx` hat ≥8 Tests: Render-Basics, Desktop-Layout, Mobile-"…"-Button, Menu-Open, Menu-Close-on-Action, Action-onClick-nach-Close, Variant-Styling, Disabled-Action.
...
- **Single-Modal-Stack-Invariant** (aus Sprint A `patterns/auth.md` / `admin-ui.md`): Action-Click schließt Menu-Modal BEVOR action.onClick. Verhindert stacked modals bei Edit-triggered DirtyConfirm oder Delete-triggered DeleteConfirm.
- **Per-Row menuOpen State** — kein globaler Zustand. Jede ListRow-Instanz hat ihren eigenen. Nie >1 Menu offen in der Praxis.
- **drag-drop-Handler** hängen am DragHandle-Element selbst (Sprint A-Pattern). ListRow passt die Handler nicht an.
