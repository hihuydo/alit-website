 ...
 *
 * Sprint C cleanup (PR #112): the legacy `session` fallback was retired
 * once the Sprint-B migration window closed. Tests now cover the
 * single-cookie-read path only.
 */
...
describe("verifySession", () => {
  ...
  it("returns {userId, tokenVersion} when cookie holds a valid token", async () => {
    const { verifySession } = await import("./auth-cookie");
    const token = await makeToken("42", JWT_SECRET, { tv: 3 });
    const result = await verifySession(
      fakeReq({ "__Host-session": token }),
    );
    expect(result).toEqual({ userId: 42, tokenVersion: 3 });
  });
  ...
  it("tokenVersion=0 for legacy JWT without tv claim (pre-T1-S)", async () => {
    ...
    expect(result).toEqual({ userId: 42, tokenVersion: 0 });
  });
  ...
  it("returns null when the cookie was signed with a different secret", async () => {
    const { verifySession } = await import("./auth-cookie");
    const bad = await makeToken("1", OTHER_SECRET);
    const result = await verifySession(
      fakeReq({ "__Host-session": bad }),
    );
    expect(result).toBeNull();
  ...
  it("returns null when the cookie is expired", async () => {
    ...
    const result = await verifySession(
      fakeReq({ "__Host-session": expired }),
    );
    expect(result).toBeNull();
  });

  it("returns null when sub is non-numeric", async () => {
    const { verifySession } = await import("./auth-cookie");
    const badSub = await makeToken("alice", JWT_SECRET);
    const result = await verifySession(
      fakeReq({ "__Host-session": badSub }),
    );
    expect(result).toBeNull();
  });

  it("ignores legacy `session` cookie in production (no fallback post-Sprint-C)", async () => {
    const { verifySession } = await import("./auth-cookie");
    const legacyToken = await makeToken("11", JWT_SECRET);
    const result = await verifySession(
      fakeReq({ session: legacyToken }),
    );
    expect(result).toBeNull();
  });
  ...
  it("returns null when no cookie is present", async () => {
    const { verifySession } = await import("./auth-cookie");
    const result = await verifySession(fakeReq({}));
    expect(result).toBeNull();
  });
...
describe("setSessionCookie", () => {
  ...
  it("sets only __Host-session in production (no legacy clear post-Sprint-C)", async () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.resetModules();
  ...
  it("sets session in dev with secure:false", async () => {
    vi.stubEnv("NODE_ENV", "development");
    vi.resetModules();
    const { setSessionCookie } = await import("./auth-cookie");
    const { res, calls } = fakeRes();
...
describe("clearSessionCookies", () => {
  ...
  it("clears session + CSRF cookies with maxAge=0 in production (no legacy clear)", async () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.resetModules();
    ...
    expect(calls.map((c) => c.name).sort()).toEqual(
      ["__Host-csrf", "__Host-session"].sort(),
    );
  ...
  it("clears session + CSRF in dev", async () => {
    vi.stubEnv("NODE_ENV", "development");
    vi.resetModules();
    const { clearSessionCookies } = await import("./auth-cookie");
    const { res, calls } = fakeRes();
...
describe("Edge-Safe guard — file content", () => {
  it("does not import Node-only modules (pg, bcryptjs, ./db, ./audit, ./auth, ./session-version)", () => {
    const filePath = path.resolve(__dirname, "auth-cookie.ts");
    const source = readFileSync(filePath, "utf8");
    const forbidden =
      /from\s+["'](pg|bcryptjs|\.\/db|\.\/audit|\.\/auth|\.\/session-version)["']/;
