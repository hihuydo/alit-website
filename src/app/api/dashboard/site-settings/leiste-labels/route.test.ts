...
const JWT_SECRET = "test-secret-at-least-32-chars-long-leiste-labels-route-aa";
...
describe("/api/dashboard/site-settings/leiste-labels/", () => {
  ...
  it("PUT 200 happy path persists labels and triggers revalidatePath", async () => {
    ...
    mockQuery.mockResolvedValueOnce({ rows: [{ token_version: 5 }] });
    mockQuery.mockResolvedValueOnce({ rows: [] }); // UPSERT result
  ...
  it("PUT 400 when string field exceeds max length", async () => {
    ...
    const res = await PUT(
      fakeReq({
        ...
        csrfCookie: csrf,
        csrfHeader: csrf,
        body: { de: { ...filledLabels, verein: "x".repeat(201) }, fr: null },
      }),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
