  ...
  return (
    <div className="flex flex-col overflow-hidden" style={{ fontFamily: "var(--font-mono)", color: "#fff" }}>
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar" style={{ fontSize: "var(--text-journal)", lineHeight: "26px" }}>
        {/* Info panel */}
        <div
          className={`overflow-hidden border-b-2 border-white transition-info ${infoVisible ? "max-h-[500px]" : "max-h-0"}`}
        ...
        {entries.map((entry, i) => (
          <div key={i} className="border-b-2 border-meta">
            ...
            <div style={{ padding: `0 var(--spacing-half) var(--spacing-half)` }}>
              {entry.title && (
                <p className={`pt-[14.667px] font-normal ${entry.titleBorder ? "border-b-3 border-black pb-[13px] mb-[13px]" : ""}`} style={{ fontSize: "var(--text-journal)" }}>
                  <strong className="font-normal" style={{ fontSize: "var(--text-journal)" }}>{entry.title}</strong>
                </p>
              )}
              {entry.lines.map((line, j) =>
                line === "" ? (
                  <p key={j} style={{ height: "26px" }} />
                ) : (
                  <p key={j} className="m-0 p-0" style={{ fontSize: "var(--text-journal)", lineHeight: "26px" }}>
